...
| 功能 | 说明 |
|------|------|
| 🎼 本地音乐播放 | 支持 12 种音频格式（详见下方格式清单），自动识别 AAC/ALAC 编码 |
...
| 🎵 歌词显示 | 自动关联 .lrc 歌词文件，播放时同步滚动高亮，点击歌词跳转 |
| 📀 CUE 分轨 | 支持 .cue 整轨专辑自动分轨入库，FLAC/APE/WAV 整轨 → 独立曲目 |
| 🌙 暗黑界面 | Aurora 风格深色主题 |
...
| 等级 | 后缀 | 编码 | 类型 | 备注 |
|------|------|------|------|------|
| 1️⃣ 必实现 | `.mp3` | MPEG-1/2 Layer-3 | 有损 | 行业通用，完整支持 VBR/CBR |
| 1️⃣ 必实现 | `.wav` | PCM | 无损 | 支持 8-48kHz，多比特深度 |
| 1️⃣ 必实现 | `.m4a` | AAC-LC / ALAC | 有损/无损 | 自动检测容器内部编码，区分 AAC vs ALAC |
| 2️⃣ 推荐 | `.flac` | FLAC | 无损压缩 | 互联网无损资源最主流 |
| 2️⃣ 推荐 | `.ogg` | Vorbis | 有损 | 开源格式 |
| 3️⃣ 可选 | `.ape` | Monkey's Audio | 无损 | 解码开销大，损坏文件容错差 |
| 3️⃣ 可选 | `.wv` | WavPack | 无损 | 小众无损 |
| 3️⃣ 可选 | `.dsf` `.dff` | DSD | HiFi | SACD 音频，软解码转 PCM 输出 |
| 3️⃣ 可选 | `.opus` | Opus | 有损 | 网络流媒体格式 |
| 3️⃣ 可选 | `.aac` | AAC | 有损 | 纯 AAC 裸流 |
| 3️⃣ 可选 | `.aiff` | PCM | 无损 | Apple 格式 |
| ❌ 不支持 | `.wma` | WMA | — | 微软旧格式，授权复杂，已移除 |

> **辅助文件**：`.cue`（整轨分轨索引）、`.lrc`（歌词文本）— 自动扫描关联，不作为独立歌曲入库
